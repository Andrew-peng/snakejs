let boxSize: number = 25;
let topScore: number = 0;

// arrow keys + WASD
let LEFT_KEYCODES: number[] = [37, 65];
let RIGHT_KEYCODES: number[] = [39, 68];
let UP_KEYCODES: number[] = [38, 87];
let DOWN_KEYCODES: number[] = [40, 83];
...
class SnakeApp {
    ...
    newFruit(): number[] {
        let fruit: number[] = [Math.floor(Math.random() * 20), Math.floor(Math.random() * 20)];
    ...
    draw() {
        ...
        this.drawSnake();
        this.drawFruit();
        (document.getElementById("score") as HTMLElement).innerHTML = "score: " + this.score;
        (document.getElementById("max-score") as HTMLElement).innerHTML = "max score: " + topScore;
        this.seenDirection = this.direction;
    }

    gameOver(): boolean {
        let pos: number[] = this.snake[0];
    ...
    moveSnake() {
        let newHead: number[] = [...this.snake[0]];
